...
def frame1():
    ...
    # labels
    title_label = Label(
        frame1, text="CRYPTOCURRENCY", font=("TimesNewRoman 40")
    ).place(x=200, y=5, width=600, height=52)
    image_label = Label(frame1, image=IMG).place(x=0, y=91)
    cryptype_label = Label(
        frame1, text="CRYPTYPE", font=("TimesNewRoman 15 bold")
    ).place(x=100, y=480, width=137, height=31)
    cryptype_label = Label(
        frame1,
        text="CURRENCY TYPE",
        font=("TimesNewRoman 15 bold")
    ).place(x=319, y=480, width=223, height=31)
    cryptype_label = Label(
        frame1, text="AMOUNT", font=("TimesNewRoman 15 bold"),
    ).place(x=618, y=480, width=112, height=31)
    cryptype_label = Label(
        frame1, text="VALUE", font=("TimesNewRoman 15 bold")
    ).place(x=813, y=480, width=87, height=31)
    # dropdown menu
    ...
    signup_button = Button(
        frame1, text="SIGN UP", command=lambda: show_frame(frame2)
    ).place(x=804, y=9, width=82, height=30)
    signin_button = Button(frame1, text="SIGN IN", command=lambda:show_frame(frame3)).place(
        x=895, y=9, width=82, height=30
    )
...
def calculation():
    ...
    if cryptype == "BTC":
        ...
        if currency_type == "INR":
            Value = Amount * CURRENT_INR_BCH
        elif currency_type == "DOLLARS":
            Value = Amount * CURRENT_DOLLAR_BCH
        elif currency_type == "EUROS":
            Value = Amount * CURRENT_EURO_BCH
    elif cryptype == "LIT":
        if currency_type == "INR":
            Value = Amount * CURRENT_INR_LIT
        elif currency_type == "DOLLARS":
            Value = Amount * CURRENT_DOLLAR_LIT
        elif currency_type == "EUROS":
            Value = Amount * CURRENT_EURO_LIT
    elif cryptype == "ETH":
        if currency_type == "INR":
            Value = Amount * CURRENT_INR_ETH
        elif currency_type == "DOLLARS":
            Value = Amount * CURRENT_DOLLAR_ETH
        elif currency_type == "EUROS":
            Value = Amount * CURRENT_EURO_ETH
...
def frame2():
    ...
    username_label = Label(
        frame2, text="USERNAME", font=("TimesNewRoman 10 bold")
    ).place(x=236, y=224, width=245, height=37)
    email_label = Label(frame2, text="EMAIL", font=("TimesNewRoman 10 bold")).place(
        x=220, y=267, width=245, height=37
    )
    password_label = Label(
        frame2, text="PASSWORD", font=("TimesNewRoman 10 bold")
    ).place(x=238, y=310, width=245, height=37)
    confirm_password_label = Label(
        frame2, text="CONFIRM PASSWORD", font=("TimesNewRoman 10 bold")
    ).place(x=269, y=353, width=245, height=37)
    ...
    create_account_button = Button(
        frame2, text="CREATE ACCOUNT", command=CA_popup
    ).place(x=450, y=422, width=150, height=35)
    back_button = Button(frame2, text="<--", command=lambda: show_frame(frame1)).place(
...
def frame3():
    ...
    back_button = Button(frame3, text="<--", command=lambda: show_frame(frame1)).place(
        x=10, y=10, width=40, height=30
    )
